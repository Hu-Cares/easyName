...
public class ItemaMallCartNumberInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object o) throws Exception {
        ...
        if (null != request.getSession() && null != request.getSession().getAttribute(Constants.MALL_USER_SESSION_KEY)) {
            ...
            ItemaMallUserVO itemaMallUserVO = (ItemaMallUserVO) request.getSession().getAttribute(Constants.MALL_USER_SESSION_KEY);
            //设置购物车中的数量
            itemaMallUserVO.setShopCartItemCount(itemaMallShoppingCartItemMapper.selectCountByUserId(itemaMallUserVO.getUserId()));
            request.getSession().setAttribute(Constants.MALL_USER_SESSION_KEY, itemaMallUserVO);
        }
        return true;
    }
