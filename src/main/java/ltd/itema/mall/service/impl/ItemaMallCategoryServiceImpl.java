...
import ltd.itema.mall.controller.vo.SecondLevelCategoryVO;
import ltd.itema.mall.controller.vo.ThirdLevelCategoryVO;
...
@Service
public class ItemaMallCategoryServiceImpl implements ItemaMallCategoryService {
    ...
    @Override
    public List<ItemaMallIndexCategoryVO> getCategoriesForIndex() {
        List<ItemaMallIndexCategoryVO> itemaMallIndexCategoryVOS = new ArrayList<>();
        //获取一级分类的固定数量的数据
        List<GoodsCategory> firstLevelCategories = goodsCategoryMapper.selectByLevelAndParentIdsAndNumber(Collections.singletonList(0L), ItemaMallCategoryLevelEnum.LEVEL_ONE.getLevel(), Constants.INDEX_CATEGORY_NUMBER);
        if (!CollectionUtils.isEmpty(firstLevelCategories)) {
            List<Long> firstLevelCategoryIds = firstLevelCategories.stream().map(GoodsCategory::getCategoryId).collect(Collectors.toList());
            //获取二级分类的数据
            List<GoodsCategory> secondLevelCategories = goodsCategoryMapper.selectByLevelAndParentIdsAndNumber(firstLevelCategoryIds, ItemaMallCategoryLevelEnum.LEVEL_TWO.getLevel(), 0);
            if (!CollectionUtils.isEmpty(secondLevelCategories)) {
                List<Long> secondLevelCategoryIds = secondLevelCategories.stream().map(GoodsCategory::getCategoryId).collect(Collectors.toList());
                //获取三级分类的数据
                List<GoodsCategory> thirdLevelCategories = goodsCategoryMapper.selectByLevelAndParentIdsAndNumber(secondLevelCategoryIds, ItemaMallCategoryLevelEnum.LEVEL_THREE.getLevel(), 0);
                    ...
                    if (!CollectionUtils.isEmpty(secondLevelCategoryVOS)) {
                        ...
                        for (GoodsCategory firstCategory : firstLevelCategories) {
                            ...
                            if (secondLevelCategoryVOMap.containsKey(firstCategory.getCategoryId())) {
                                ...
                                itemaMallIndexCategoryVO.setSecondLevelCategoryVOS(tempGoodsCategories);
                                itemaMallIndexCategoryVOS.add(itemaMallIndexCategoryVO);
                            }
                        }
                    }
                }
            }
            return itemaMallIndexCategoryVOS;
        } else {
            return null;
        }
    }
    ...
    public SearchPageCategoryVO getCategoriesForSearch(Long categoryId) {
        ...
        if (thirdLevelGoodsCategory != null && thirdLevelGoodsCategory.getCategoryLevel() == ItemaMallCategoryLevelEnum.LEVEL_THREE.getLevel()) {
            //获取当前三级分类的二级分类
            GoodsCategory secondLevelGoodsCategory = goodsCategoryMapper.selectByPrimaryKey(thirdLevelGoodsCategory.getParentId());
            if (secondLevelGoodsCategory != null && secondLevelGoodsCategory.getCategoryLevel() == ItemaMallCategoryLevelEnum.LEVEL_TWO.getLevel()) {
                //获取当前二级分类下的三级分类List
                List<GoodsCategory> thirdLevelCategories = goodsCategoryMapper.selectByLevelAndParentIdsAndNumber(Collections.singletonList(secondLevelGoodsCategory.getCategoryId()), ItemaMallCategoryLevelEnum.LEVEL_THREE.getLevel(), Constants.SEARCH_CATEGORY_NUMBER);
