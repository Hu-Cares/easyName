...
public class ItemaMallCartNumberInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object o) throws Exception {
        ...
        if (null != request.getSession() && null != request.getSession().getAttribute(Constants.MALL_USER_SESSION_KEY)) {
            ...
            ItemaUserVO itemaUserVO = (ItemaUserVO) request.getSession().getAttribute(Constants.MALL_USER_SESSION_KEY);
            //设置购物车中的数量
            itemaUserVO.setShopCartItemCount(itemaMallShoppingCartItemMapper.selectCountByUserId(itemaUserVO.getUserId()));
            request.getSession().setAttribute(Constants.MALL_USER_SESSION_KEY, itemaUserVO);
        }
        return true;
    }
