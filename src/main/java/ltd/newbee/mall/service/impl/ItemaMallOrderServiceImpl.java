...
import ltd.newbee.mall.service.ItemaMallOrderService;
...
public class ItemaMallOrderServiceImpl implements ItemaMallOrderService {
    ...
    public String saveOrder(ItemaUserVO user, Long couponUserId, List<ItemaMallShoppingCartItemVO> myShoppingCartItems) {
        ...
        List<ItemaMallGoods> itemaMallGoods = itemaGoodsMapper.selectByPrimaryKeys(goodsIds);
        // 检查是否包含已下架商品
        ...
        if (!CollectionUtils.isEmpty(goodsListNotSelling)) {
            ...
            ItemaException.fail(goodsListNotSelling.get(0).getGoodsName() + "已下架，无法生成订单");
        }
        ...
        for (ItemaMallShoppingCartItemVO shoppingCartItemVO : myShoppingCartItems) {
            ...
            if (!newBeeMallGoodsMap.containsKey(shoppingCartItemVO.getGoodsId())) {
                ItemaException.fail(ServiceResultEnum.SHOPPING_ITEM_ERROR.getResult());
            }
            // 存在数量大于库存的情况，直接返回错误提醒
            if (shoppingCartItemVO.getGoodsCount() > newBeeMallGoodsMap.get(shoppingCartItemVO.getGoodsId()).getStockNum()) {
                ItemaException.fail(ServiceResultEnum.SHOPPING_ITEM_COUNT_ERROR.getResult());
            }
        }
        if (CollectionUtils.isEmpty(itemIdList) || CollectionUtils.isEmpty(goodsIds) || CollectionUtils.isEmpty(itemaMallGoods)) {
            ItemaException.fail(ServiceResultEnum.ORDER_GENERATE_ERROR.getResult());
        }
        if (itemaMallShoppingCartItemMapper.deleteBatch(itemIdList) <= 0) {
            ItemaException.fail(ServiceResultEnum.DB_ERROR.getResult());
        }
        List<StockNumDTO> stockNumDTOS = BeanUtil.copyList(myShoppingCartItems, StockNumDTO.class);
        int updateStockNumResult = itemaGoodsMapper.updateStockNum(stockNumDTOS);
        ...
        for(ItemaMallGoods o: itemaMallGoods) {
            ...
            if (priceTotal < 1) {
                ItemaException.fail(ServiceResultEnum.ORDER_PRICE_ERROR.getResult());
            }
    ...
    public String seckillSaveOrder(Long seckillSuccessId, Long userId) {
        ...
        if (!itemaMallSeckillSuccess.getUserId().equals(userId)) {
            throw new ItemaException("当前登陆用户与抢购秒杀商品的用户不匹配");
        }
        ...
        ItemaMallGoods itemaMallGoods = itemaGoodsMapper.selectByPrimaryKey(goodsId);
        // 生成订单号
        String orderNo = NumberUtil.genOrderNo();
        // 保存订单
        ItemaMallOrder itemaMallOrder = new ItemaMallOrder();
        ...
        itemaMallOrder.setExtraInfo(extraInfo);
        if (itemaMallOrderMapper.insertSelective(itemaMallOrder) <= 0) {
            throw new ItemaException("生成订单内部异常");
        }
        // 保存订单商品项
        ItemaMallOrderItem itemaMallOrderItem = new ItemaMallOrderItem();
    ...
    public ItemaMallOrderDetailVO getOrderDetailByOrderNo(String orderNo, Long userId) {
        ...
        if (!userId.equals(itemaMallOrder.getUserId())) {
            ItemaException.fail(ServiceResultEnum.NO_PERMISSION_ERROR.getResult());
        }
        ...
        itemaMallOrderDetailVO.setNewBeeMallOrderItemVOS(itemaMallOrderItemVOS);
            ...
            if (!CollectionUtils.isEmpty(orderIds)) {
                ...
                for (ItemaMallOrderListVO itemaMallOrderListVO : orderListVOS) {
                    ...
                    if (itemByOrderIdMap.containsKey(itemaMallOrderListVO.getOrderId())) {
                        ...
                        itemaMallOrderListVO.setNewBeeMallOrderItemVOS(itemaMallOrderItemVOS);
                    }
                }
            }
        }
