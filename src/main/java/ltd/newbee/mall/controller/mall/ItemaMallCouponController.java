...
public class ItemaMallCouponController {
    ...
    public String couponList(HttpServletRequest request, HttpSession session) {
        ...
        if (session.getAttribute(Constants.MALL_USER_SESSION_KEY) != null) {
            userId = ((ItemaUserVO) request.getSession().getAttribute(Constants.MALL_USER_SESSION_KEY)).getUserId();
        }
    ...
    public String myCoupons(HttpServletRequest request, HttpSession session) {
        ItemaUserVO userVO = (ItemaUserVO) session.getAttribute(Constants.MALL_USER_SESSION_KEY);
